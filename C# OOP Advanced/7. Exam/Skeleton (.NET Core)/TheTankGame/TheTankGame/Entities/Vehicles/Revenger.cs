using System;
using System.Collections.Generic;
using System.Text;
using TheTankGame.Entities.Miscellaneous;
using TheTankGame.Entities.Miscellaneous.Contracts;

namespace TheTankGame.Entities.Vehicles
{
    public class Revenger : BaseVehicle
    {
        private IAssembler vehicleAssembler = new VehicleAssembler();

        public Revenger(string model, double weight, decimal price, 
            int attack, int defense, int hitPoints,IAssembler assembler) 
            : base(model, weight, price, attack, defense,
                  hitPoints,assembler)
        {
           
        }
    }
}
